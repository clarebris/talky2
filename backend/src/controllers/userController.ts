...
import { execute, query } from "../dbHelpers/dbconnect";
import { v4 as uuidv4 } from "uuid";
import { updatUser, user } from "../types/userInterfaces";
import { generateToken } from "../dbHelpers/tokenGenerator";
import {
  validateLoginUser,
...
import { comparePass, hashPass } from "../dbHelpers/passwordHash";
...
export const updateUser = async (req: Request, res: Response) => {
  try {
    const { _id, username, email, profileImage, fullName } = req.body;

    const { error } = validateUpdateuser.validate(req.body);
    if (error)
      return res
        .status(400)
        .send({ error: "check your username and email " });
